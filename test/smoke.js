...
(async () => {
  ...
  await wait(800);
  assert(q3('#msgsheet').classList.contains('hidden') && spoken.length === 0, 'a new message is not read out or popped up by itself (nobody may be home)');
  assert(q3('#msg-badge').textContent === '2' && qa3('#msgs .mrow')[0].classList.contains('unread'), 'badge counts unconfirmed messages: ' + q3('#msg-badge').textContent);
  assert(!q3('#frame-msgs').classList.contains('hidden') && q3('#frame-msgs').textContent === '💬 2 nye beskjeder', 'photo frame badge: ' + q3('#frame-msgs').textContent);
  click3(q3('#messages-panel'));
  assert(!q3('#msgsheet').classList.contains('hidden'), 'tapping the panel opens the Beskjeder sheet');
  ...
  assert(!items3[0].querySelector('.confirm') && !items3[1].querySelector('.confirm'), 'no «Hørt!» before a message has been heard');
  click3(items3[0].querySelector('.mplay'));
  await wait(20);
  ...
  await wait(30);
  assert(q3('#msg-badge').classList.contains('hidden') && q3('#frame-msgs').classList.contains('hidden') && ack3().length === 2 && hook3().length === 2, 'all confirmed: badges gone, second ✅ and #hjemme post sent');
