...
- **Separate backend, not in `index.html`.** A small Node service — on the **Ubuntu screen PC** as a systemd service (`call-backend/`, port 3000), reached by the kiosk same-origin at `/api` via nginx — owns the LiveKit API secret, the Discord bot token (reuse GameNoobBot's), the call state, one-time answer tokens and short-lived LiveKit JWTs. The kiosk keeps every `CLAUDE.md` rule: one file, still works from `file://`; the LiveKit client is lazy-loaded from a CDN only during a call. LiveKit itself runs on Unraid.
- **Domains** (SWAG + Cloudflare, proxied): `rtc.noobventure.com` → LiveKit `:7880` on Unraid (WSS); `call.noobventure.com` → the backend on the **Ubuntu PC:3000** (SWAG upstream = that PC's LAN IP) for the parent answer page. **UDP 7882** forwarded WAN → Tower for WebRTC media (two hops; see SETUP.md §2 for the double-router note).
...
**Implemented so far (2026-09-10):** the `call-backend/` service (Express + `livekit-server-sdk` + Discord DM over REST) with `POST /api/calls`, `/api/calls/answer`, `/api/calls/:id/cancel|end`, `GET /api/calls/:id`, the parent answer page (`public/answer.html`, `livekit-client` from CDN), `docker-compose.yml` (LiveKit only, on Unraid) + `livekit.example.yaml`, `deploy/menkerud-backend.service` (systemd) and `deploy/nginx-menkerud.conf`, the two SWAG confs, plus `README.md` and the top-level **`DEPLOYMENT.md`** (source of truth for deployment). In `index.html`, `startCall(who)` → `startCallLiveKit` + `loadLiveKit` (lazy CDN); `cfg.call.backend` defaults to `''` (same-origin `/api`), with `cfg.call.deviceKey` / `cfg.call.timeoutSec`; Jitsi is gone. **Not yet done:** deploy per DEPLOYMENT.md, real media test on mobile data, and a proper parent→screen (incoming) flow over LiveKit (today “Svar” on an incoming call simply calls the parent back).
