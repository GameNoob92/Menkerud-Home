...
| `scripts/kiosk-screen.py` + `call-backend/deploy/menkerud-screen.service` | Kiosk screen helper: a user-level Python service on `127.0.0.1:7777` that blacks out the screen for Skjerm av (backlight 0 via logind `SetBrightness`; DPMS only as a fallback) and restores it on the first touch; installed per SETUP.md §3. |
