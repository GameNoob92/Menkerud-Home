...
```js
{
  ...
  messages: { autoSpeak },                            // Beskjeder: read a new Discord message aloud as it arrives (false = chime + badge, the child taps)
...
**Beskjeder** → a parent writes in the Discord channel `msgChannel` → gateway `MESSAGE_CREATE` → `onNewMessage`: stored, BESKJEDER panel + badge, chime, `#msgsheet` opens with the message as number 1 and the screen says «Ny beskjed fra Mor: …» (`heard`). The child taps **✅ Hørt!** → `read`, ✅ reaction on the message, «@Mor ✅ Beskjeden er hørt (14:32): «…»» in #hjemme, toast «Sendt til Mor 💌». Tapping the panel opens the sheet with the last 10; tapping a row replays it; `MESSAGE_DELETE` in Discord removes it. Night / screen off / during a call: nothing is read, the badge waits for a tap.
